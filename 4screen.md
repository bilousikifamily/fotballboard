...
3. **Контейнер стадій (один `div.admin-layout__body`)** — займає `1fr` висоти, чотири колонки `grid-template-columns: 10% 40% 40% 10%` і три вертикальні рядки: `22% / 70% / 8%`.
   ...
   - Ряд `70%` — зона логотипів і `score-controls`, під ними розміщений `div.admin-layout__score-probability` з ймовірністю рахунку (менший шрифт/вага; ховається, якщо немає odds для рахунку).

   - Нижній ряд (`8%`) — блок голосування `div.admin-layout__vote` з кнопкою `Проголосувати`.
