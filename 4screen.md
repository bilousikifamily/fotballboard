...
3. **Контейнер стадій (один `div.admin-layout__body`)** — займає `1fr` висоти, чотири колонки `grid-template-columns: 10% 40% 40% 10%` і три вертикальні рядки: `22% / 70% / 8%`.
   ...
   - Нижній ряд (`8%`) — блок голосування `div.admin-layout__vote` з `div.admin-layout__score-probability` над кнопкою `Проголосувати`; якщо користувач вже проголосував — блоки ховаються.
